...
const Calculatrice = () => {
    ...
    const handleNumberChange = (e) => {
        let temp = number == undefined ? e.target.innerHTML : number + e.target.innerHTML;
        let temp2 = number2 == undefined ? e.target.innerHTML : number2 + e.target.innerHTML;
    ...
    return (
        <div>
            <div>
                {array.map((i, index) => (
                    <button onClick={handleNumberChange} key={index}>{i}</button>
                ))}
                <br/>
